...
async function requirePath(path, label) {
  try { await access(path, constants.R_OK); } catch { throw new NativeBuildError('native_toolchain_mismatch', `${label} is missing at ${path}.`); }
}
...
export async function inspectNativeToolchain(options = {}) {
  ...
  const faustVersion = await output('faust', ['--version']);
  ...
  await Promise.all([
    ...
    requirePath('/opt/homebrew/include/faust/dsp/dsp.h', 'Faust headers'),
    requirePath(resolve(iPlug2Root, 'iPlug2.cmake'), 'iPlug2'),
    requirePath(resolve(vst3SdkRoot, 'CMakeLists.txt'), 'VST3 SDK'),
  ]);
  const mismatches = [
    !faustVersion.includes(lock.faust.version) && `Faust ${lock.faust.version}`,
  ...
  if (mismatches.length) throw new NativeBuildError('native_toolchain_mismatch', `Native toolchain mismatch: ${mismatches.join(', ')}.`);
  return { ok: true, generator: lock.cmake.generator, paths: { repositoryRoot: root, iPlug2Root, vst3SdkRoot, validatorPath } };
}
