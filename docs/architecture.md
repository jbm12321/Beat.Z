...
faust/                            fourteen canonical versioned primitive sources
...
Durable `ProjectV2` data includes IDs, name, monotonic revision, exact Faust provenance, connected order, all nodes, Controls (the internal `macros` field), activity, and any legacy recovery record. Playback, selected panels, meters, local-file bytes/name, comparison renders, validation results, proposals, frozen-build UI, and WebMCP support state are transient.
