  ...
  return (
    <div className="mapping-row">
      ...
      <div className="range-fields">
        <label>Min<input type="number" min={definition.min} max={definition.max} step={definition.step} defaultValue={mapping.min} key={`${mapping.id}-min-${mapping.min}`} onBlur={(event) => onUpdate({ min: Number(event.target.value) })} /></label>
        <label>Max<input type="number" min={definition.min} max={definition.max} step={definition.step} defaultValue={mapping.max} key={`${mapping.id}-max-${mapping.max}`} onBlur={(event) => onUpdate({ max: Number(event.target.value) })} /></label>
