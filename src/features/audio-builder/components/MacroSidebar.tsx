...
export function MacroSidebar({ project, mobileOpen, selectedMacroId, selectedMacro, hasAvailableTarget, onCreateMacro, onSelectMacro, onAddMapping, onChangeMappingTarget, onCommit, onHide }: {
  ...
  return (
    <aside className={`macro-sidebar ${mobileOpen ? 'is-mobile-open' : ''}`} aria-label="Controls">
      ...
      {project.macros.length === 0 ? (
        <div className="macro-empty">
          ...
          <p>Map DSP parameters to controls</p>
          <button type="button" className="outline-button" onClick={onCreateMacro}>Create Controls</button>
        </div>
      ) : (
          ...
          <div className="macro-grid">
            {project.macros.map((macro) => (
              <ProjectKnob
                ...
                onRename={(name) => onCommit([{ type: 'rename_macro', macroId: macro.id, name }])}
                onDelete={() => onCommit([{ type: 'delete_macro', macroId: macro.id }])}
              />
            ))}
